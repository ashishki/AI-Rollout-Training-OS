#!/usr/bin/env bash
# Checkpoint helper: save_checkpoint.sh
# Writes a lightweight Orchestrator state snapshot when a host environment signals session end.
#
...
Resume: read docs/prompts/ORCHESTRATOR.md in the current Codex session.
...
if [ "${SILENT:-0}" != "1" ] \
  ...
  NOTIFY_TEXT="Session ended ${TIMESTAMP}. Active: ${ACTIVE_TASK}. Fix Queue: ${FIX_COUNT}. Resume from docs/prompts/ORCHESTRATOR.md."
